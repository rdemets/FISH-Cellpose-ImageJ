...
// Open file if CZI
	...
	if(endsWith(currFile, ".czi")) { // process czi files matching regex
		...
		run("Cellpose ...", "env_path=E:\\Anaconda3\\envs\\env-cellpose env_type=conda model=cyto3 model_path=path\\to\\own_cellpose_model diameter=30 ch1=0 ch2=-1 additional_flags=[--use_gpu, --flow_threshold, 0]");
...
//			run("Split Channels");
//			run("Merge Channels...", "c1=[C1-Image_max] c2=[C2-Image_max] c4=[C4-Image_max] c5=[C5-Image_max] create");
//			channels = 4;
//		}
		...
		for (ch = 2; ch <= channels; ch++) {
			...
			if (ch==2) {
				...
				run("Find Maxima...", "prominence="+prominence4+" output=[Single Points] light");
			}
			else {
				run("Find Maxima...", "prominence="+prominence5+" output=[Single Points] light");
			}
			...
			for(j=0;j<NNuc;j++){
				...
				if (ch==2) {
					...
					Spot_ch3 = Array.concat(Spot_ch3, NSpots);
				}
				else {
					Spot_ch4 = Array.concat(Spot_ch4, NSpots);
				}
			}
			close("Spot");
			// Display results
...
Array.show(ImageTitle, CellNumber,Spot_ch1, Spot_ch2, Spot_ch3, Spot_ch4);	
Dialog.create("Done");
Dialog.show();
